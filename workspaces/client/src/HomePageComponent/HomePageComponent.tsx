...
export function HomepageComponent() {
  ...
  if (
    !document.cookie.split(";").find((row) => row.startsWith("token="))?.length
  ) {
    console.log("not here");
    navigate("/");
  }
  const { approved, pending } = useAppSelector(
    (root: RootState) => root.LoginSlice
  );
  const { products } = useAppSelector((root: RootState) => root.ProductSlice);
  const searchWord = useAppSelector(
    (root: RootState) => root.SearchSlice.value
  );
  ...
  const matchPercent = (product: any, searchWord: any) => {
    ...
    let intersection = name
      .split("")
      .filter((x: any) => searchWord.split("").includes(x));
  ...
  return approved ? (
    ...
    `}
      ...
      <Container className="cards-view">
        {products
          ?.filter((product) => matchPercent(product, searchWord) >= 50)
          .map((product) => (
            <CardComponent
              path={product?.path}
